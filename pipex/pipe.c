...
/*   Updated: 2023/01/25 15:41:25 by gduhau           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	first_pipe(t_minishell *elem, t_all *p, t_tree *start)
{
	...
	if (elem->pid == 0)
	{
		...
		if (close(elem->fd[0]) == -1 || close (elem->fd[1]) == -1 || cond_redir(elem) == -1 || g_sig.sig_int > 0 || g_sig.sig_quit > 0 || exec_command(maj_path(p->env), elem->cmd, p, start) != 0)
			end_process(p, 1);
		exit(0);
	}
